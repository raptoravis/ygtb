...
default_fig = dict(
    data=[{"x": [], "y": []}],
    layout=dict(
        xaxis=dict(range=[], visible=False),
        yaxis=dict(range=[0, 1000], color="white"),
...
def update_data(intervals):
    connection = sqlite3.connect("./_data/ws_data.db")
    cursor = connection.cursor()

    time_from = math.floor((time.time() - 60) * 1000)

    data = cursor.execute(
        f"SELECT * FROM trades WHERE time > {time_from} ORDER BY time DESC"
    ).fetchall()

    current_price = data[0][3]
    total_trades = len(data)

    # print((dict(x=[[time.time()]], y=[[total_trades]]), [0], 100), current_price)
